...
|---|---|---|---|
| 1 | **UI speaks Bootstrap, not the host** — remeasured 20 Aug | **The headline number was counting the wrong thing.** Most Bootstrap-looking classes in plugin templates are SPACING AND FLEX UTILITIES — `mb-3`, `mt-4`, `d-flex`, `text-muted`, `w-100` — which `theme.css` defines and the host's OWN templates use. Shared vocabulary, not drift; converting them is churn. `--bs-warning` is likewise an alias the host maintains. Counting **components only** — card, btn, badge, alert, row/col, table-striped, nav-link, form-control — gives **3,072 hits across 93 of 105 template files**, and **71 uncaptioned tables**, not 38. Worst: requests 533 (another workstream), usenet 398, roadmap 383, donations 298, releasegroups 249. | **Nothing is broken** — the host ships shims deliberately, and theme.css says so above `.card`: "legacy demo markup drops content straight into .card". The visible tell is components rendering Bootstrap's palette beside the site's own. The recipe, from the `store` conversion (20 Aug, 71 → 0): `card`/`card-body`→`panelV2`/`panel__body`, `row`+`col-md-6`→`grid grid--2`, `btn btn-primary`→`button button--primary`, `badge bg-*`→`tag tag--*`, `alert alert-*`→`notice notice--*`, `form-label`/`form-control`→`form__label`/`form__input`, `var(--bs-*)`→the real token, tables→`data-table` in a `data-table-wrapper` with a `visually-hidden` caption. **Watch `h-100`**: `.panelV2` is `height:fit-content` where `.card` is not, so dropping it silently breaks equal-height rows and any `mt-auto` inside them — caught by screenshot, invisible in the diff. |
| 2 | **Admin surfaces invisible to the hub** — ~~18 ✗~~ **0 (20 Aug)** | measured against the running site rather than counted from source: **17 admin routes** were served and reachable from no nav — donations, forum, messages, news, store, wiki, plus the host's own `/admin/features` and `/admin/metrics`. | The prescription was half wrong. `SlotAdminPage` mounts ONE GET plus POST actions, so a plugin with editors (wiki, donations, news) cannot be that view and should not be contorted into one. It contributes a LINK instead — `pluginapi.RegisterAdminNav`, new 20 Aug. `scripts/audit_adminnav.py` keeps it at zero. **Linking them found a page that had been broken the whole time**: `/admin/donate` truncated mid-render on `index` with a `*int` key, unseen because no crawler could reach it. |
| 3 | **Nothing to build on: no events** — measured 20 Aug: **39** plugins declared none, not 29 | but **zero declared-but-never-fired**, so the `lists.created` bug class is clean — 23 declared, 23 emitted. Six filled since: `comments.posted`/`.thanked`, `cosmetics.unlocked`/`.equipped`, `polls.voted`, `hitrun.warned`, `mediainfo.report.posted`/`.shot.added`, `applications.submitted`/`.decided` (**33 declared now**). The directory core builds is finally RENDERED, on /admin/contracts — it existed and was invisible. | `DeclareEvent` the notable facts with **honest flags** — achievements consumes any countable event with no code change of its own, so a wrong `Countable` is the whole cost. The four added are deliberately a spread of the judgement: cosmetics declares one countable and one not for the same member action; hitrun's is a member event nobody should score on; polls' payload omits the option because the results policy exists to control when a tally is visible. Still bare and worth doing: medals (its granter holds only the store, so emitting needs plumbing), magic, games, uploads, communities, releasegroups, downloads, news. Infrastructure plugins (backups, dbmaint, logs, curation, stats, feeds, catalog, predb) have nothing member-facing to announce and should stay quiet. |
...
| 7 | **Job hygiene** — ~~pause skipped on manual triggers~~ **FIXED CENTRALLY 20 Aug** | measured: **33 triggers across 18 plugins** ignored pause, not 9 — but the other two items did not survive checking. | **The fix was one gate, not 33 edits.** `schedule.TriggerJob` gated on already-running and on the write gate and not on paused; the pause check now sits beside them, so every plugin is covered and one written tomorrow is too (`loon@fce264f`). Verified live: pause a job, press Run now, the log says *Manual trigger REFUSED: paused by admin*. <br>**`SetIdle(time.Time{})` is NOT a defect** — `ServiceLoop` calls `AnnounceNextRun` after every tick, deliberately overriding what a tick function set, so a zero horizon is overwritten before anyone sees it; the live jobs page shows no zero times. discord's is a persistent service with genuinely no next run. <br>**Interval literals**: 11 inline at `RunLoop` call sites, 8 plugins already name a constant. Each literal is used once at its own site, so there is nothing repeated to collapse. Left alone. |
