...
| 3 | **Nothing to build on: no events** — measured 20 Aug: **39** plugins declared none, not 29 | but **zero declared-but-never-fired**, so the `lists.created` bug class is clean — 23 declared, 23 emitted. Four filled since: `comments.posted`/`.thanked`, `cosmetics.unlocked`/`.equipped`, `polls.voted`, `hitrun.warned` (29 declared now). | `DeclareEvent` the notable facts with **honest flags** — achievements consumes any countable event with no code change of its own, so a wrong `Countable` is the whole cost. The four added are deliberately a spread of the judgement: cosmetics declares one countable and one not for the same member action; hitrun's is a member event nobody should score on; polls' payload omits the option because the results policy exists to control when a tally is visible. Still bare and worth doing: medals, magic, games, mediainfo, uploads, applications, communities, releasegroups, downloads, news. Infrastructure plugins (backups, dbmaint, logs, curation, stats, feeds, catalog, predb) have nothing member-facing to announce and should stay quiet. |
