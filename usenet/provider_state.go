...
// missedArticleRetryLimit is how many times a number that keeps not being
// returned is re-requested before it is written off.
//
// Gaps are overwhelmingly legitimate — RFC 3977 s6 makes article numbers sparse
// and holes permanent — so the point of retrying at all is to separate "expired,
// never coming back" from "the server did not include it in that response".
// Four attempts, spread across whatever passes happen to run, is enough to make
// that distinction without accumulating work on groups whose history is simply
// gone.
const missedArticleRetryLimit = 4

// reconcileMissedArticles brings the ledger in line with one fetched range.
//
// Two halves, in one transaction, because doing either alone is wrong:
//
//   - numbers in [lo,hi] that came back are DELETED. This is what makes the
//     table shrink instead of growing forever, and it is how a re-offered
//     article is recovered rather than merely counted.
//   - numbers that did not come back are upserted with attempts+1, so a number
//     missing once is distinguishable from one missing across many passes hours
//     apart — the difference between a server that omitted a line and history
//     that is genuinely gone.
//
// The delete is expressed as "in range AND NOT in the missing list" rather than
// "in the returned list", so the caller only has to carry the small slice.
// Deleting the whole range first and re-inserting would reset attempts on every
// pass and destroy the only signal the table exists to produce.
func (s *PGStore) reconcileMissedArticles(ctx context.Context, backbone, group string, lo, hi int64, missing []int64) error {
	if lo > hi {
		lo, hi = hi, lo
	}
	return s.db.WithTx(ctx, func(tx *sqlx.Tx) error {
		if _, err := tx.ExecContext(ctx,
			`DELETE FROM missed_articles
			  WHERE backbone = $1 AND group_name = $2
			    AND number BETWEEN $3::bigint AND $4::bigint
			    AND NOT (number = ANY($5::bigint[]))`,
			backbone, group, lo, hi, pq.Array(missing)); err != nil {
			return err
		}
		if len(missing) == 0 {
			return nil
		...
		capped := missing
		if len(capped) > missedArticlesPerBatchCap {
			capped = capped[:missedArticlesPerBatchCap]
		}
		_, err := tx.ExecContext(ctx,
			`INSERT INTO missed_articles (backbone, group_name, number)
			 SELECT $1, $2, n FROM unnest($3::bigint[]) AS n
			 ON CONFLICT (backbone, group_name, number)
			 DO UPDATE SET attempts = missed_articles.attempts + 1, last_try = now()`,
			backbone, group, pq.Array(capped))
		return err
	})
}

// missedArticlesPerBatchCap bounds what one pathological batch can write.
// Generous relative to a normal gap, small relative to a range walking expired
// history — which is exactly the case that should be written off rather than
// recorded article by article.
const missedArticlesPerBatchCap = 5000

// missedArticleStats reports the outstanding and written-off counts for a group
// — the numbers an operator needs to decide whether the omissions are a provider
// problem or ordinary expiry.
func (s *PGStore) missedArticleStats(ctx context.Context, backbone, group string) (outstanding, writtenOff int64, err error) {
	err = s.db.WithTx(ctx, func(tx *sqlx.Tx) error {
		return tx.QueryRowContext(ctx,
			`SELECT count(*) FILTER (WHERE attempts < $3),
			        count(*) FILTER (WHERE attempts >= $3)
			   FROM missed_articles WHERE backbone = $1 AND group_name = $2`,
			backbone, group, missedArticleRetryLimit).Scan(&outstanding, &writtenOff)
	})
