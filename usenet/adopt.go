...
//
// Coverage is deliberately NOT carried. It was, briefly (migrations 020/021,
// reverted by 022): recording the legacy watermarks' span as fetched made the
// coverage strip look right and left the group contiguously covered, which
// makes the BACKFILL find no gaps and mark itself done. That silently blocks
// re-reading a span the old crawler only partly indexed — and production's
// figures show it missing whole months. An empty coverage map understates what
// is in the catalogue; a full one blocks its repair, and only one of those is
// recoverable.
//
