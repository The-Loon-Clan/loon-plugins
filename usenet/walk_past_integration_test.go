...
func TestSweepWalkPastEvictsOnlyTheDead(t *testing.T) {
	...
	stage("Dead.Set", 2, 5, 100)     // short, padded span [90,111] inside coverage
	...
	stage("Frontier.Set", 2, 5, 295) // span [295,296]: coverage ends at 300, inside the margin — the walk has NOT gone past it

	for _, base := range []string{"Dead.Set", "Uncovered.Set", "Complete.Set", "Spanless.Set", "Frontier.Set"} {
		backdate(t, r, group, base, time.Hour)
	}

	cov := map[string][]articleRange{group: {{Start: 50, End: 300}}}
	scanned, evicted, _, err := r.sweepWalkPast(ctx, cov, 15*time.Minute, 10_000, 0, 10)
	if err != nil {
	...
	for _, base := range []string{"Graced.Set", "Uncovered.Set", "Complete.Set", "Spanless.Set", "Frontier.Set"} {
		if !exists(base) {
...
func TestSweepWalkPastIsBoundedAndConverges(t *testing.T) {
	...
	_, evicted, _, err := r.sweepWalkPast(ctx, cov, 15*time.Minute, 100, 0, 10)
	if err != nil {
		t.Fatal(err)
	}
	if evicted == 0 {
	...
	for i := 0; i < 20; i++ {
		if _, _, _, err := r.sweepWalkPast(ctx, cov, 15*time.Minute, 100, 0, 10); err != nil {
			t.Fatal(err)
		}
