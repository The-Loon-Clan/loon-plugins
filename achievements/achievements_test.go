...
// paid_at says whether the payment landed. A member who earned something
// whose reward has not settled has still earned it — reporting that as locked
// would tell them they had not. (In the old shared-schema design the second
// fact was the grant's state; paid_at replaced it when the plugins split.)
func TestAchievementRowMapsCompletionAndPaymentToPageState(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name: "completed and settled",
			row: achievementRow{Slug: "hundred", Threshold: 100,
				CompletedAt: sql.NullTime{Time: earned, Valid: true},
				PaidAt:      sql.NullTime{Time: earned, Valid: true}},
			want: AchievementUnlocked, dated: true,
		},
		{
			// The crash window, or a granter outage: the badge is held, the
			// payment is owed, and the repair sweep will bring it. Pending is
			// the honest word for that.
			name: "completed, payment not yet recorded",
			row: achievementRow{Slug: "hundred", Threshold: 100,
				CompletedAt: sql.NullTime{Time: earned, Valid: true}},
...
func TestAchievementRowCarriesProgressAndCriterion(t *testing.T) {
	got := achievementRow{
		...
		Icon: "star", ImagePath: "/img/badge.png",
		Progress: sql.NullInt64{Int64: 63, Valid: true},
	...
	// The look rides along — the field set is the contract hosts read through
	// the extension, Icon and ImagePath included.
	if got.Icon != "star" || got.ImagePath != "/img/badge.png" {
		t.Errorf("icon/image lost: %q / %q", got.Icon, got.ImagePath)
	}
}
