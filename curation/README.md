...
1. **title** — the host's canonical parser found a marker in the release
   title itself.
2. **non-seasonal** — movies, OVAs and specials keep NULL on purpose; the
   display layer buckets them by keyword, and a written season would relabel
   a movie as "Season 1" on every surface.
3. **meta-mapped** — the community anidb→tvdb/tmdb mapping (Fribb
   anime-lists, via the host's ID map) says outright which season of its
   mapped show this AniDB entry is. Strongest metadata rule: it resolves
   sequels whose names carry no ordinal at all ("...: The Final Season").
4. **meta-ordinal** — the AniDB entry the release is tagged to is named as a
   ...
   authoritative.
5. **meta-single-season** — TMDB (via the host's `completion_buckets` season
   cache) says the show has exactly one season and the entry is a seasonal
   format: it can only be season 1.
6. **unresolved** — nothing applied; the row stays NULL and is listed.
