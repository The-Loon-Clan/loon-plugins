...
- **Templates are the plugin's**, embedded under `templates/`: `community_forums.html`, `community_category.html`, `community_thread.html`, `community_new_thread.html`, `admin_forum_categories.html`. The host used to carry all 1,300 lines, which meant the plugin could not change its own pages and the site could not delete them. The host supplies chrome through `RenderPage`. An earlier contract let a host render its own copies by name instead; it was removed once the demo stopped using it, and `Provision` now refuses anything but the full set.
