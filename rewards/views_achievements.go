...
func (p *Plugin) actionCreateAchievement(gc *gin.Context) (template.HTML, error) {
	if p.admin == nil {
		return p.redirect(gc, "achievements", "", "no admin store configured")
	}
	if err := gc.Request.ParseForm(); err != nil {
		return p.redirect(gc, "achievements", "", "could not read the form")
	}
	a, err := parseNewAchievement(gc.Request.PostForm)
	if err != nil {
		return p.redirect(gc, "achievements", "", err.Error())
	}
	...
	// and deliberate act.
	return p.redirect(gc, "achievements",
...
func (p *Plugin) actionToggleAchievement(gc *gin.Context) (template.HTML, error) {
	...
	if err != nil || id <= 0 {
		return p.redirect(gc, "achievements", "", "bad achievement id")
	}
	on := gc.PostForm("on") == "1"
	...
	return p.redirect(gc, "achievements", "Disabled. Progress is kept; nothing new completes.", "")
}
