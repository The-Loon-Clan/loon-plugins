...
// The fragment must execute against the real view model. html/template streams,
// so a field the model lacks truncates the card mid-row and returns 200.
func TestProfileCardRendersWithRealData(t *testing.T) {
	...
	// A pending badge says so: the member holds it but has not been paid, and
	// silently showing it as earned invites "where are my points".
...
func TestInProgressIsSelfOnly(t *testing.T) {
	...
	m.Rewards = append(m.Rewards, Reward{ID: 1, Slug: "r", Kind: KindOneOff, Enabled: true,
		Payouts: []Payout{{Kind: "points", Amount: 1}}})
	d := m.SeedAchievement(AchievementDef{
		Slug: "veteran", Name: "Veteran", Metric: "m", Threshold: 100, RewardID: 1, Enabled: true,
	})
	if _, err := m.RecordProgress(nil, d.ID, 5, 63); err != nil {
		t.Fatal(err)
	}

	// The rendering decision lives in the handler, so assert on the view model
	// the handler would build rather than on HTML.
	all, err := m.Achievements(nil, 5)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Viewer is a stranger: no in-progress rows, and with nothing earned there
