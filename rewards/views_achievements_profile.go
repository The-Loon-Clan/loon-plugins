...
//
// Until this, achievements were complete end to end and invisible: the plugin
// published rewards.achievements, the host consumed nothing, and a member could
// earn one without any surface ever saying so. This is the surface.
//
...
type profileAchievement struct {
	...
	// locked achievement. Clamped: a metric that overshoots (615 of 500) would
...
type profileVM struct {
	...
	Pending    int
	Locked     int
	// Self is true when the viewer is looking at their own profile. Progress on
	// something not yet earned is shown only to the member themselves: on
	// someone else's profile it is a list of things they have failed to do.
	Self bool
}

func (p *Plugin) registerAchievementProfile(c *core.Core) error {
	return c.RegisterView(core.View{
		Slug: "achievements", Title: "Achievements", Slot: core.SlotUserWidget,
		// Public: earned badges are a public fact about a member — that is what
		// makes them worth earning. The in-progress half is gated below.
		Public: true,
...
func (p *Plugin) renderProfileAchievements(gc *gin.Context) (template.HTML, error) {
	...
	// Nothing to say: render nothing rather than an empty card. An "no
	// achievements yet" box on every profile forever is how a card gets ignored
	// before it ever has content — the same reasoning as the claim widget.
...
// unit-tested rather than asserted about through rendered HTML: which badges are
// public, which progress is private, what a hidden achievement does before it is
// earned, and what an overshooting counter does to a progress bar.
func buildProfileVM(all []Achievement, self bool) profileVM {
	...
	for _, a := range all {
		...
		if a.Threshold > 0 {
			...
			if pct > 100 {
				// A metric read can legitimately overshoot (615 of 500). Without
				// this the bar renders wider than its track.
				pct = 100
			}
			if pct < 0 {
				pct = 0
		...
		// Only the member sees what they are partway through, and only if they
		// have actually started: a full list of everything unearned is not a
		// profile card, it is a chore list.
